...
import { updateUserProgress } from "../services/userProgressService"; // Import updateUserProgress
...
const Bedroom = () => {
  ...
  const [staminaInterval, setStaminaInterval] = useState(null); // State to manage interval
  ...
  useEffect(() => {
    if (isAsleep) {
      const interval = setInterval(async () => {
        if (auth.currentUser) {
          try {
            await updateUserProgress(auth.currentUser.uid, { stamina: increment(1) });
          } catch (error) {
            console.error("Error updating stamina:", error);
          }
        }
      }, 1000); // Increase stamina every second
      setStaminaInterval(interval);
    } else if (staminaInterval) {
      clearInterval(staminaInterval);
      setStaminaInterval(null);
    }
